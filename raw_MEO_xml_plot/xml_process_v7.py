...
def xml_process(filename, filetype, country= 'USA', searchtag = False, searchstr = False):
    packets = []
    ...
    if searchstr: # could use this statement with the default bcnID = False to process all or using search string
        ...
        print packets
    elif (filetype == 'packet' or filetype == 'TOA_FOA_DATA'):
...
DASSAntMC INT, DASSAnt INT)''')
    ...
    if (filetype == "solutionsMessage"):
        rowlist = []
        for event, elem in packets:
            #print elem.tag, elem.text
            rowlist.append(elem.text)
            #print rowlist
        print len(rowlist)
        #print rowlist
        burstlist.extend(rowlist)
        #print burstlist
        #print burstlist
        #print len(burstlist)
...
def search_zip_output(MEOLUT, zip,zipmatches,filetypesearch,csvoutfile, country = 'USA', searchtag = None,searchstr = None, writeto = 'both'):
    ...
    with zipfile.ZipFile(zip, 'r') as myzip:
        for files in zipmatches:
            ...
            packets = xml_process(file,filetypesearch, country, searchtag,searchstr)
...
country = str(configdict['Country'])
...
if filetypesearch == 'solution': filetypesearch = 'solutionsMessage'
...
for day in datelist:
    for MEO in MEOLUT_list:
        ...
        print 'zipmatches'
        print len(zipmatches)
        burstlist = list()
        #Main function to search zip files and output to csv or sql
        search_zip_output(MEO, zipf, zipmatches, filetypesearch, csvoutfile, country, searchtag1,searchstr1, writeto)
        if (writeto == 'sql_db' or writeto == 'both') and (filetypesearch == 'packet' or filetypesearch == 'TOA_FOA_DATA'):
    ...
    Unknown1 INT, Unknown2 INT, CN0 REAL, BitRate REAL, Unknown3 INT, UNIQUE(MEOLUT, Ant, Time))''')
            ...
            if country == 'Canada':    
                def sliceit(x): return x[:12]
                burstlist = map(sliceit,burstlist)    
