...
# - add notes to config file - done
# - use datetime instead of year/month/day for start and end dates - i think this is done now
# - Make different csv output that only writes errors
# - Make csv output for status messages correct (it will have to go to a sql db first it seems) 
...
for day in datelist:
    for MEO in MEOLUT_list:
        ...
        if (writeto == 'sql_db' or 'both') and (filetypesearch == 'packets' or 'TOA_FOA_DATA'):
