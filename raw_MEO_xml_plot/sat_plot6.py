...
### 12/19/16 - Working - plotting singular value, multiple comma separate values or default to all values for antenna


def find_packets(sql_file, beaconid = '%', start_date = 0, end_date = None, MEOLUT = '%',ant_i = '%', sat = '%'):
    conn = sqlite3.connect(sql_file)
    c = conn.cursor()
    if MEOLUT <> '%': MEO = MEOdict[MEOLUT]
    else: MEO = '%'
    query = ['%'+beaconid+'%', MEO, start_date, end_date, ant_i, sat]
...
def norad(sat):
    ...
    try: norad = data[ind[0]][1] 
        ...
        print "Satellite ' " + sat + " ' is not listed in 'sats.csv'" 
        return 0 
    return norad
...
def plot_sat(Xi,sat,start_time,stop_time,delta,MEOLUT,freq0):
    ...
    for times in timelist:
        ...
        X_i_ECEF, V_i_ECEF = sgp4lib.TEME_to_ITRF(datetime1,array(position),array(V_i_TE)) # get satellite pos and vel
        posilist.append(X_i_ECEF)
        V_i_ECEF = [x/86400 for x in V_i_ECEF] # convert back to #km/sec
        velolist.append(V_i_ECEF)
        Rig = (MEOLUTLocDict[MEOLUT] - X_i_ECEF) # Rig is vector from MEOLUT to satellite
        Rig_list.append(Rig)
        Rig_dot = dot(V_i_ECEF,(Rig/norm(Rig)))
        
        Ri = (Xi -X_i_ECEF) # Xi is the beacon location, Ri is the vector from beacon to satellite
        Ri_list.append(Ri)
        ...
        # testing here 
        dF_ig = -(Rig_dot/c)*freq0 if inverted == True else (Rig_dot/c)*freq0
        dF_i =  (Ri_dot/c)*(freq_down-dF_ig) #if inverted == True else (Ri_dot/c)*freq0
...
#Dictionaries
MEOdict = {'Hawaii':3385,'Florida':3669,'Maryland':3677,'ShirleysBay':3165}
...
3165:array((1093.633, -4354.632, 4515.049))
}
MEOList = [3669, 3385, 3677, 3165]
AntList = range(1,7)
MEOref = {3669:'ADDC002%', 3385:'AA5FC00%', 3677:'ADDC002%',3165:'A79EEE26E32E190'}
...
###START OF SCRIPT          
    ...
    #ant_list = int(configdict['antenna']) if configdict['antenna'] <> '' else AntList
    ant_list = configdict['antenna'] if configdict['antenna'] <> '' else AntList
    if isinstance(ant_list, basestring):
        ant_list = list(map(int,ant_list.split(','))) 
    elif isinstance(ant_list, float):
        ant_list = [int(ant_list)]
    ...
    if beaconidstr == "Hawaii_Ref": 
        ...
        ref_flag = True
        num_bursts = num_bursts*3
    if beaconidstr == "Ottawa_Ref": 
        beaconidstr = "A79EEE26E32E190"
        ref_flag = True
